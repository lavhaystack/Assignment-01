...
async function fetchUsers() {
  ...
  try {
    const res = await fetch(`http://localhost:3000/api?results=${count}`);
...
function renderUsers() {
  ...
  usersData.forEach((user, index) => {
    ...
    countryRandom += `<p ondblclick="openModal(${index})">${user.location.city}, ${user.location.country}</p>`;
  });
...
function openModal(index) {
  ...
  document.getElementById("modalFields").innerHTML = `
    ...
    <div class="bubble-field" contenteditable="false" id="modalAddress">${user.location.city}, ${user.location.state}, ${user.location.country}</div>
...
document.getElementById("nameType").addEventListener("change", renderUsers);
